...
export DATA=~/../../projects/ensa/plants/RNAseq/Begfuc
...
# It is important to create the index out of this code by running #hisat2-build ${GENOME} ${GENOME%.*} # This is to create the index. It should be done out of the code.
#Check rnaseq_hisatindex.sh
...
ncbi/${key}${SLURM_ARRAY_TASK_ID}_2.fastq.gz --outdir ${DATE}

#trim paired end 
bbduk.sh  in1=${key}${SLURM_ARRAY_TASK_ID}_1.fastq.gz \
in2=${key}${SLURM_ARRAY_TASK_ID}_2.fastq.gz \
...
#pair end reads
hisat2 -p 20 -x ${INDEX} -1 ${key}${SLURM_ARRAY_TASK_ID}.clean_1.fastq.gz -2 \
${key}${SLURM_ARRAY_TASK_ID}.clean_2.fastq.gz -S ${key}${SLURM_ARRAY_TASK_ID}.sam 
