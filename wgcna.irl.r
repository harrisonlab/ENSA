...
inp <- opt$gene_table
outdir <- "~/data/wgcna_out"

# Load input file
exp_data<-read_delim("filename", delim=".")
...
if (!gsg$allOK){
    ...
    if (sum(!gsg$goodGenes) > 0){
        ...
        gfile <- paste("~/data/wgcna_out", "removed_genes.txt", sep = "/")
        write(genes_to_remove, file = gfile)
    }
    if (sum(!gsg$goodSamples) > 0){
        samples_to_remove <- (paste("Removing samples:",
        paste(rownames(datexpr0)[!gsg$goodSamples], collapse = "\n")))
        sfile <- paste("~/data/wgcna_out", "removed_samples.txt", sep = "/")
...
file <- paste("~/data/wgcna_out", "sample_clustering.pdf", sep = "/")
...
Rfile <- paste("~/data/wgcna_out", "Cleaned_data.RData", sep = "/")
...
lfile <- paste("~/data/wgcna_out", "Cleaned_data.RData", sep = "/")
...
cex1 <- 0.9
file <- paste("~/data/wgcna_out", "sft_testing.pdf", sep = "/")
...
file <- paste("~/data/wgcna_out", "clustering_tree.pdf", sep = "/")
pdf(file, height = 9, width = 12)
sizeGrWindow(12, 9)
...
file <- paste("~/data/wgcna_out", "clustering_tree_with_modules.pdf", sep = "/")
pdf(file, height = 9, width = 12)
...
file <- paste("~/data/wgcna_out", "clustering_tree_with_merged_modules.pdf", sep = "/")
pdf(file, height = 9, width = 12)
...
file <- paste("~/data/wgcna_out", "clustering_tree_compare_modules.pdf", sep = "/")
pdf(file, height = 9, width = 12)
...
mes <- mergedmes
Rfile <- paste("~/data/wgcna_out", "modules.RData", sep = "/")
...
for (i in unique(modulecolours)){
  ...
  write.csv(ME_all[[i]], file=paste0("~/data/wgcna_out","/genes_",i,".csv"),
            row.names = F)
}
